...
const columns: ColumnDef<BannerType>[] = [
  {
    ...
    header: () => <div className="text-center">Зураг</div>,
    cell: ({ row }) => {
      const bannerUrl = row.getValue("bannerUrl") as {
        asset: {
          _id: string;
          url: string | null;
        } | null;
      ...
      return (
        <div className="px-3">
          <Avatar className="rounded-lg w-15">
            <AvatarImage
              src={bannerUrl?.asset?.url || ""}
              alt={"preview"}
...

  {
    ...
    header: "Гарчиг",
    cell: ({ row }) => {
      return <div className="w-50 truncate">{row.getValue("title")}</div>;
    },
    enableHiding: false,
  },
  {
    ...
    header: "Дэд гарчиг",
    cell: ({ row }) => (
      <div className="w-50 truncate">{row.original.subTitle}</div>
    ),
  },
  {
  ...
  },
  {
    id: "actions",
    cell: ({ row }) => {
      const addBanner = useBanner((state) => state.addBanner);
